...
const ClaveProductSearch = ({ value, onChange, placeholder = "Search Clave Producto", label = "Clave Producto" }) => {
  ...
  const searchClaveProd = async (query) => {
    ...
    try {
      const { data } = await http.get(`/api/financials/clave-unidad/search?query=${encodeURIComponent(query)}`);
      console.log('Search results:', data);
      // Handle the response structure with results array
      setSearchResults(data.results || []);
    } catch (error) {
      console.error('Error searching clave producto:', error);
      setSearchResults([]);
    } finally {
...
export default ClaveProductSearch;
